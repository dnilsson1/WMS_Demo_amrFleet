    ...
    <form on:submit|preventDefault={handleCreateOrder} class="space-y-4">
      {#each orderItems as item, i}
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label for="product-select" class="block">Product:</label>
            <select id="product-select" bind:value={item.product_id} class="w-full border p-2 rounded">
          ...
          <div>
            ...
            <input id="product-quantity" type="number" bind:value={item.quantity} min="1" class="w-full border p-2 rounded" />
          </div>
        </div>
      {/each}
